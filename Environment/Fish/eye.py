...
class Eye:
    ...
    def __init__(self, board, verg_angle, retinal_field, is_left, num_arms, min_distance, max_distance, dark_gain,
        ...
        self.readings = cp.zeros((num_arms, 3), 'int')
        self.board = board
    ...
    def get_repeated_computations(self):
        ...
        channel_angles_surrounding = cp.repeat(channel_angles_surrounding, self.n, 1)
        rf_offsets = cp.linspace(-self.photoreceptor_rf_size / 2, self.photoreceptor_rf_size / 2, num=self.n)
        self.channel_angles_surrounding = channel_angles_surrounding + rf_offsets

        mul_for_hypothetical = cp.array([[1, 0], [0, 1], [1, 0], [0, 1]])
        self.mul_for_hypothetical = cp.tile(mul_for_hypothetical, (self.photoreceptor_num, self.n, 1, 1))
        add_for_hypothetical = cp.array(
            [[0, 0], [0, 0], [0, self.width - 1], [self.height - 1, 0]])
        self.add_for_hypothetical = cp.tile(add_for_hypothetical, (self.photoreceptor_num, self.n, 1, 1))

        mul1 = cp.array([0, 0, 0, 1])
        self.mul1_full = cp.tile(mul1, (self.photoreceptor_num, self.n, 1))

        addition_matrix_unit = cp.array([0, 0, self.height - 1, self.width - 1])
        self.addition_matrix = cp.tile(addition_matrix_unit, (self.photoreceptor_num, self.n, 1))

        conditional_tiled = cp.array(
            [self.width - 1, self.height - 1, self.width - 1, self.height - 1])
        self.conditional_tiled = cp.tile(conditional_tiled, (self.photoreceptor_num, self.n, 1))

        multiplication_matrix_unit = cp.array([-1, 1, -1, 1])
        self.multiplication_matrix = cp.tile(multiplication_matrix_unit, (self.photoreceptor_num, self.n, 1))
    ...
    def update_angles(self, verg_angle, retinal_field, is_left):
        if is_left:
            ...
            min_angle = np.pi / 2 - retinal_field / 2 - verg_angle / 2
            max_angle = np.pi / 2 + retinal_field / 2 - verg_angle / 2
        self.vis_angles = cp.linspace(min_angle, max_angle, self.num_arms)
    ...
    def read(self, masked_arena_pixels, eye_x, eye_y, fish_angle):
        ...
        valid_intersection_coordinates = cp.reshape(valid_intersection_coordinates, (self.photoreceptor_num, self.n, 2, 2))

        # Get intersections (PR_N x 2)
        eye_position = cp.array([eye_x, eye_y])
        ...
        selected_intersections = cp.reshape(selected_intersections, (self.photoreceptor_num, self.n, 1, 2))

        eye_position_full = cp.tile(eye_position, (self.photoreceptor_num, self.n, 1, 1))
        ...
        # SEGMENT COMPUTATION
        x_lens = cp.rint(max_x[:, 0] - min_x[:, 0])
        y_lens = cp.rint(max_y[:, 0] - min_y[:, 0])
        ...
        full_set = cp.vstack((set_1, set_2)).astype(int)
        ...
        oversampling_ratio = cp.expand_dims(oversampling_ratio, 1)
        oversampling_ratio = cp.repeat(oversampling_ratio, 3, 1)
    ...
    def get_sector_vertices(self, eye_x, eye_y, fish_angle):
        ...
        eye_position_full = cp.tile(eye_position, (self.photoreceptor_num, 1, 1))
        vertices = cp.concatenate((eye_position_full, relevant_intersections_1, relevant_intersections_2), axis=1)
