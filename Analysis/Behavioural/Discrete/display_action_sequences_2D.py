...
def plot_action_sequences_2D_discrete(fish_positions_compiled, actions_compiled, action_sequence_timestamps,
    ...
    """Given postiion and action choice data, plots action sequences in 2D field."""

    fish_positions_compiled_flattened = np.array(fish_positions_compiled)
    fish_positions_compiled_flattened = np.reshape(fish_positions_compiled_flattened, (-1, 2))

    actions_compiled_flattened = np.array(actions_compiled)
    actions_compiled_flattened = np.reshape(actions_compiled_flattened, (-1))

    action_sequence_timestamps_flattened = np.array(action_sequence_timestamps)
