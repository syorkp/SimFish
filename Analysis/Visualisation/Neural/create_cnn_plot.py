...
data1 = load_data("ppo_no_sigma-5", "New_Assay_test", "Environment-1")
# data2 = load_data("even_prey_ref-5", "For-Traces", "Prey-Static-10")
n_c_neurons = data1["actor_conv4r"].shape[-1]

# a_neuron = [data["left_conv_4"][i, 0, :, 0] for i in range(len(data["left_conv_4"]))]

left_neurons = [[data1["actor_conv4r"][i, 0, :, j] for i in range(len(data1["actor_conv4r"]))] for j in range(n_c_neurons)]
