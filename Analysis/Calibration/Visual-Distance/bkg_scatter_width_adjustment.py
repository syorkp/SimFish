"""Script to find how to scale background_brightness value depending on width of enviornment (and decay), to prevent visibility
...
def compute_sum_of_bkground(width, background_brightness, visual_range):
    ...
    total_scatter = np.sum(scatter * background_brightness)
...
def build_bk_scatter_model(background_brightness, visual_range):
    widths = np.linspace(100, 10000, 100).astype(int)
    widths2 = np.linspace(100, 10000, 100).astype(int)
    scatters = [compute_sum_of_bkground(width, background_brightness, visual_range=visual_range) for width in widths]
    ...
    # background_brightness_adjustment = (p(width)/p(1500)) * background_brightness
