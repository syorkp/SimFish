...
class PPONetwork:
    ...
    def __init__(self, simulation, rnn_dim_shared, rnn_dim_critic, rnn_dim_actor, rnn_cell_shared, rnn_cell_critic,
        ...
        self.log_prob_impulse_placeholder = tf.log(self.norm_dist_impulse.prob(tf.math.divide(self.action_placeholder[0][0], max_impulse)) + 1e-5)

        self.old_log_prob_angle_placeholder = tf.placeholder(shape=[None], dtype=tf.float32, name='old_log_prob_angle')
        self.log_prob_angle_placeholder = tf.log(self.norm_dist_angle.prob(tf.math.divide(self.action_placeholder[0][1], max_angle_change)) + 1e-5)
