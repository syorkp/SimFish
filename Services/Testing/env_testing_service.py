...
class EnvTestingService(BaseService):
    ...
    def __init__(self, config_name, continuous_actions):
        ...
        self.visualisation_board = VisualisationBoard(self.environment_params["width"],
                                                      self.environment_params["height"],
                                                      light_gradient=self.environment_params["light_gradient"])
        self.board_fig, self.ax_board = plt.subplots()
        self.board_image = plt.imshow(np.zeros((self.environment_params['height'], self.environment_params['width'], 3)))
        plt.ion()
        plt.show()

    def controlled_episode_loop(self):
        ended = False
        while not ended:
            print("Enter action: ")
            if self.continuous_actions:
    ...
    def update_drawing(self):
        ...
        self.draw_shapes()
        self.visualisation_board.apply_light(dark_col=int(self.environment_params["dark_light_ratio"] * self.environment_params["width"]),
    ...
    def draw_shapes(self):
        scaling_factor = 1500 / self.environment_params["width"]
        prey_size = self.environment_params['prey_size_visualisation']/scaling_factor
        ...
        self.visualisation_board.fish_shape(self.simulation.fish.body.position,
                                            self.environment_params['fish_mouth_size'],
                                            self.environment_params['fish_head_size'],
        ...
        if len(self.simulation.prey_bodies) > 0:
            ...
            rrs = np.clip(rrs, 0, self.environment_params["width"]-1)
            ccs = np.clip(ccs, 0, self.environment_params["height"]-1)
            ...
            try:
                ...
                if max(rrs.flatten()) > self.environment_params['height']:
                    lost_index = np.argmax(py)
                elif max(ccs.flatten()) > self.environment_params['width']:
        ...
        if len(self.simulation.sand_grain_bodies) > 0:
            ...
            try:
                ...
                if max(rrs.flatten()) > self.environment_params['width']:
                    lost_index = np.argmax(px)
                elif max(ccs.flatten()) > self.environment_params['height']:
                    lost_index = np.argmax(py)
                else:
                    lost_index = 0
                    print(f"Fix needs to be tuned: PX: {max(px)}, PY: {max(py)}")
        ...
        for i, pr in enumerate(self.simulation.predator_bodies):
            self.visualisation_board.circle(pr.position, self.environment_params['predator_size'],
                                            self.simulation.predator_shapes[i].color, True)

        if self.simulation.predator_body is not None:
            self.visualisation_board.circle(self.simulation.predator_body.position, self.environment_params['predator_size'],
    ...
    def output_frame(self, internal_state=np.array([[0, 0, 0]]), scale=0.25):
        ...
        arena[0, :, 0] = np.ones(self.environment_params['width']) * 255
        arena[self.environment_params['height'] - 1, :, 0] = np.ones(self.environment_params['width']) * 255
        arena[:, 0, 0] = np.ones(self.environment_params['height']) * 255
        arena[:, self.environment_params['width'] - 1, 0] = np.ones(self.environment_params['height']) * 255

        empty_green_eyes = np.zeros((20, self.environment_params["width"], 1))
        ...
        left_eye = resize(np.reshape(left_photons, (1, self.simulation.fish.left_eye.observation_size, 3)) * (
                255 / 100), (20, self.environment_params['width'] / 2 - 50))
        ...
        print(f"Red max: {np.max(eyes[:, :, 0])}")
        print(f"UV max: {np.max(eyes[:, :, 1])}")
        print(f"Red2 max: {np.max(eyes[:, :, 2])}")
        print()
        eyes = np.concatenate((eyes[:, :, :1], empty_green_eyes, eyes[:, :, 1:2]),
                                  axis=2)  # Note removes second red channel.

        frame = np.vstack((arena, np.zeros((50, self.environment_params['width'], 3)), eyes))

        this_ac = np.zeros((20, self.environment_params['width'], 3))
        this_ac[:, :, 0] = resize(internal_state, (20, self.environment_params['width']), anti_aliasing=False,
                                  order=0) * 255
        this_ac[:, :, 1] = resize(internal_state, (20, self.environment_params['width']), anti_aliasing=False,
                                  order=0) * 255
        this_ac[:, :, 2] = resize(internal_state, (20, self.environment_params['width']), anti_aliasing=False,
                                  order=0) * 255

        frame = np.vstack((frame, np.zeros((20, self.environment_params['width'], 3)), this_ac))
