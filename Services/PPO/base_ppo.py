            ...
            if os.path.isfile(f"{self.model_location}/rnn_state-{self.episode_number}.json"):
                if self.environment_params["use_dynamic_network"]:
                    with open(f"{self.model_location}/rnn_state-{self.episode_number}.json", 'r') as f:
                        ...
                        print(num_rnns)
                        self.init_rnn_state_actor = tuple((np.array(data[f"rnn_state_{shape}_1"]), np.array(data[f"rnn_state_{shape}_2"])) for shape in range(num_rnns))
                        self.init_rnn_state_actor_ref = tuple((np.array(data[f"rnn_state_ref_{shape}_1"]), np.array(data[f"rnn_state_{shape}_ref_2"])) for shape in range(num_rnns))
