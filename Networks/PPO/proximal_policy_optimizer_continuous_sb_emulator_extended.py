...
class PPONetworkActorMultivariate2Extended(BaseNetwork):
    ...
    def __init__(self, simulation, rnn_dim, rnn_cell, my_scope, internal_states, max_impulse, max_angle_change,
        ...
        if input_sigmas:
            ...
            self.impulse_stream_sigma_2 = tf.layers.dense(self.impulse_stream_sigma, self.impulse_stream_sigma.shape[0], activation=tf.nn.sigmoid,
                                          kernel_initializer=tf.orthogonal_initializer,
                                          name=my_scope + '_impulses2', trainable=True)

            self.angle_stream_sigma_2 = tf.layers.dense(self.angle_stream_sigma, self.angle_stream_sigma.shape[0], activation=tf.nn.sigmoid,
                                          kernel_initializer=tf.orthogonal_initializer,
                                          name=my_scope + '_angles_2', trainable=True)
            self.impulse_stream_sigma_ref_2 = tf.layers.dense(self.impulse_stream_sigma_ref, self.impulse_stream_sigma_ref.shape[0],
                                                          activation=tf.nn.sigmoid,
                                                          kernel_initializer=tf.orthogonal_initializer,
                                                          name=my_scope + '_impulses2', trainable=True, reuse=True)

            self.angle_stream_sigma_ref_2 = tf.layers.dense(self.angle_stream_sigma_ref, self.angle_stream_sigma_ref.shape[0],
                                                        activation=tf.nn.sigmoid,
                                                        kernel_initializer=tf.orthogonal_initializer,
        ...
        self.angle_stream_2 = tf.layers.dense(self.angle_stream, self.angle_stream.shape[0], activation=tf.nn.sigmoid,
                                      kernel_initializer=tf.orthogonal_initializer,
                                      name=my_scope + '_angle_2', trainable=True)
        self.impulse_stream_ref_2 = tf.layers.dense(self.impulse_stream_ref, self.impulse_stream_ref.shape[0],
                                                      activation=tf.nn.sigmoid,
                                                      kernel_initializer=tf.orthogonal_initializer,
                                                      name=my_scope + '_impulse2', trainable=True, reuse=True)

        self.angle_stream_ref_2 = tf.layers.dense(self.angle_stream_ref, self.angle_stream_ref.shape[0],
                                                    activation=tf.nn.sigmoid,
                                                    kernel_initializer=tf.orthogonal_initializer,
                                                    name=my_scope + '_angle_2', trainable=True, reuse=True)

        #            ----------        Mu Estimations       ---------            #

        self.mu_impulse = tf.layers.dense(self.impulse_stream_2, 1, activation=tf.nn.sigmoid,
                                          kernel_initializer=tf.orthogonal_initializer,
                                          name=my_scope + '_mu_impulse', trainable=True)
        self.mu_impulse_ref = tf.layers.dense(self.impulse_stream_ref_2, 1, activation=tf.nn.sigmoid,
                                              kernel_initializer=tf.orthogonal_initializer,
                                              name=my_scope + '_mu_impulse', trainable=True, reuse=True)

        self.mu_angle = tf.layers.dense(self.angle_stream_2, 1, activation=tf.nn.tanh,
                                        kernel_initializer=tf.orthogonal_initializer,
                                        name=my_scope + '_mu_angle', trainable=True)
        self.mu_angle_ref = tf.layers.dense(self.angle_stream_ref_2, 1, activation=tf.nn.tanh,
                                            kernel_initializer=tf.orthogonal_initializer,
        ...
        if input_sigmas:
            ...
            self.sigma_impulse = tf.layers.dense(self.impulse_stream_sigma_2, 1, activation=tf.nn.sigmoid,
                                                 kernel_initializer=tf.orthogonal_initializer,
                                                 name=my_scope + '_sigma_impulse', trainable=True)
            self.sigma_impulse = self.bounded_output(self.sigma_impulse, 0, max_sigma)

            self.sigma_angle = tf.layers.dense(self.angle_stream_sigma_2, 1, activation=tf.nn.sigmoid,
                                               kernel_initializer=tf.orthogonal_initializer,
                                               name=my_scope + '_sigma_angle', trainable=True)
            self.sigma_angle = self.bounded_output(self.sigma_angle, 0, max_sigma)

            self.sigma_impulse_ref = tf.layers.dense(self.impulse_stream_sigma_ref_2, 1, activation=tf.nn.sigmoid,
                                                     kernel_initializer=tf.orthogonal_initializer,
            ...
            self.sigma_angle_ref = tf.layers.dense(self.angle_stream_sigma_ref_2, 1, activation=tf.nn.sigmoid,
                                                   kernel_initializer=tf.orthogonal_initializer,
